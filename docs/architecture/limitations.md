...
| **A second physics library for 2D** | Declined outright, and **P7 landed the alternative**: `ColliderFreedom::Plane2D` is one enum field that becomes Jolt's `EAllowedDOFs`, and the unit suite pins the plane coordinate to an exact equality over 300 steps. [`Physics.h`](../../engine/scene/Physics.h) states there is no second solver behind `Impl`. Adopting Box2D beside Jolt would be two solvers to keep deterministic, for a feature Jolt has |
